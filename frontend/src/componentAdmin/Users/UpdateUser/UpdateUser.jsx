...
const UpdateUser = () => {
    ...
    const [userData, setUserData] = useState({
        name: '', email: '', password: '', confirmPassword: '', gender: '', isAdmin: ''
    });
    const { id } = useParams()
    ...
    const handleSubmit = (e) => {
        ...
        setErr(err)
        if (valid) {
            dispatch(updatedUser({ _id: userData._id, name: userData.name, email: userData.email, password: userData.password, gender: changeBoolean(userData.gender), isAdmin: changeBoolean(userData.isAdmin) }))
        }
    }
    ...
    useEffect(() => {
        ...
        if (user) {
            ...
            setUserData({ ...user, password: '', confirmPassword: '', gender: gender1, isAdmin: isAdmin1 })
        }
    }, [dispatch, id, user])
        ...
        <Container component="main" maxWidth="xs">
            ...
            <div className={classes.paper}>
                ...
                {loading ? (
                    ...
                    {updateError && <Alert style={{ marginTop: '10px' }} severity="error">{updateError}</Alert>}
                    ...
                    <Grid container spacing={1} style={{ marginTop: "5px" }}>
                        <Grid item xs="12" md="6">
                            <Button component={changeURL} to="/admin/user/" onClick={() => dispatch({ type: USER_ADMIN_DETAILS_RESET })} variant="outlined" fullWidth>Go Back</Button>
                        </Grid>
                        <Grid item xs="12" md="6">
