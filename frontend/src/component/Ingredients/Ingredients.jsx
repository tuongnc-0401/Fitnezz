import { Box, CircularProgress, Grid, Paper, Snackbar, TextField, Typography, InputLabel, Select, MenuItem, Link } from '@material-ui/core';
...
const Ingredients = () => {
    ...
    if (userBMI) {
        var caloriesBMI = !userBMI.caloGainLoss ? Math.ceil(parseInt(userBMI.numTDEE) / 3) : Math.ceil(parseInt(userBMI.caloGainLoss) / 3)
    }
        ...
        return (
            <Box mt={3} m={3}>
                <Alert severity="error">
                    ...
                    <Link component={goBackBMI} to="/ingredients">
                        ...
                        <>
                            ...
                            <Grid container spacing={3} className={classes.gridContainer}>
                                ...
                                <Grid item xs={12} sm={6} md={4} lg={3} className={classes.fixed}>
                                                ...
                                                <Paper style={{ display: "flex", margin: '10px' }} elevation={2} >
                                                    ...
                                                    <Box style={{ flexGrow: '1' }} p={1} >
                                                        ...
                                                        <Box display="flex" alignItems="center" mt={2}>
                                                            <input className={classes.input} type="number" style={{ width: '50px', marginRight: 'auto' }} value={ingredient.qty === 0 ? null : ingredient.qty}
                                                                onChange={(e) => {
                                    ...
                                    {loadingBMI ? <CircularProgress color="secondary" />
                                        : errorBMI ? <Alert severity="error">{error}</Alert> :
                                            total >= caloriesBMI ?
                                                <Alert style={{ marginTop: '10px' }} severity="warning">Please make your meal under {caloriesBMI} calories</Alert>
                                                :
                                                <Paper style={{ marginTop: '5px', padding: '10px' }}>
                                                    <Typography variant="h6">Your body status is <span style={{ color: '#f73471' }}>{userBMI.statusBMI}</span></Typography>
                                                    {/* <Typography>{userBMI.caloGainLoss} + {userBMI.numTDEE} + {userBMI.caloGainLoss}</Typography> */}
                                                    <Typography variant="h6">Please make your meal under <span style={{ color: '#f73471' }}>{caloriesBMI}</span> calories</Typography>
                                                </Paper>
                                    }
                                </Grid>
                            </Grid>
